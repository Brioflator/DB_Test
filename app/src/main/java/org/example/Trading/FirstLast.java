    ...
    void update(Trade trade)
    {
        if (firstTrade == null || firstTrade.time().isAfter(trade.time()) || firstTrade.time().equals(trade.time())) {
            if (firstTrade != null && firstTrade.time().equals(trade.time())) {
                logger.warning(String.format("Warning: Two trades have the same time: %s. First trade ticker: %s, Second trade ticker: %s", trade.time(), firstTrade.ticker(), trade.ticker()));
        ...
        if (lastTrade == null || lastTrade.time().isBefore(trade.time()) || lastTrade.time().equals(trade.time())) {
            if (lastTrade != null && lastTrade.time().equals(trade.time())) {
                logger.warning(String.format("Warning: Two trades have the same time: %s. First trade ticker: %s, Second trade ticker: %s", trade.time(), lastTrade.ticker(), trade.ticker()));
